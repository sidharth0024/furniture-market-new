<?php
/**
 * index.php — Furniture Market Homepage (fully database-driven)
 *
 * Architecture:
 *   index.php → layouts/header.php → Admin_panel/includes/db_conn.php ($pdo)
 *                                  → includes/functions.php
 *             → layouts/navbar.php → includes/fetch-categories.php
 *             → layouts/footer.php
 *
 * All data comes from the database. No hardcoded product or category data.
 */

require_once 'layouts/header.php';
?>
<title>Furniture Market – Premium Furniture for Every Space</title>
      ...
      <div class="prod-tab-panel <?= $ti === 0 ? 'active' : '' ?>" id="featTab<?= (int)$tc['id'] ?>">
        <div class="prod-slider-wrap" id="featWrap<?= (int)$tc['id'] ?>">
          <div class="prod-slider" id="featSlider<?= (int)$tc['id'] ?>"
            ...
            <?php
            if ($ti === 0) {
                foreach ($firstTabProducts as $p) { echo productCard($p); }
            }
            ?>
          </div>
      ...
      <div class="col-lg-9 col-md-8">
        ...
        <div class="prod-slider-wrap" id="catWrap<?= (int)$cat['id'] ?>">
          <div class="prod-slider" id="<?= $trackId ?>"
               style="flex-wrap:nowrap;transition:transform 350ms ease;">
            <?php foreach ($prods as $p): echo productCard($p); endforeach; ?>
          </div>
          <div class="prod-slider-controls">
...
<section class="section-trust">
  <div class="container-fluid px-4">
    ...
    $trustCards = [
        ...
        ['bi-shop',        'Trusted Marketplace', 'Thousands of buyers source furniture through Furniture Market every month.'],
...
function makeSlider(trackEl, progressEl) {
    ...
    function getCards() { return trackEl.querySelectorAll('.product-card-col'); }
    ...
    function getMetrics() {
        ...
        if (!cards.length || !wrapEl) {
            return { cards: cards, step: 0, maxScroll: 0, maxIndex: 0 };
        }
    ...
    function maxIndex() { return getMetrics().maxIndex; }
...
function initSlider(trackId) {
    ...
    if (wrap) {
        addSwipe(wrap,
            function() { sliders[trackId].next(); },
            function() { sliders[trackId].prev(); }
        );
    }
...
document.querySelectorAll('[data-feat-next]').forEach(function(btn) {
    ...
    btn.addEventListener('click', function() { initSlider(trackId).next(); });
});
document.querySelectorAll('[data-feat-prev]').forEach(function(btn) {
    var trackId = btn.getAttribute('data-feat-prev');
    btn.addEventListener('click', function() { initSlider(trackId).prev(); });
});
