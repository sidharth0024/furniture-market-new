...
if (!$product) {
  ...
  echo '<title>Product Not Found – Furniture Shoppers</title>';
      ...
      <div class="col-lg-6 col-md-12">
        <div class="product-info">
          ...
          <p class="product-meta-line mb-3">
            ...
            <?php endif; ?>
            <span class="pm-item">Brand: <strong>Furniture Shoppers</strong></span>
          </p>
...
<script>
  ...
  (function() {
    ...
    var IMAGES = <?= json_encode(array_values(array_map(function ($g) {
                    return $g['src'];
    ...
    if (gPrev) gPrev.addEventListener('click', function() {
      setImage((activeIdx - 1 + IMAGES.length) % IMAGES.length);
    });
    if (gNext) gNext.addEventListener('click', function() {
      setImage((activeIdx + 1) % IMAGES.length);
    });
    ...
    if (tUp && tvp) tUp.addEventListener('click', function() {
      tvp.scrollBy({
        top: -80,
        behavior: 'smooth'
      });
    });
    if (tDown && tvp) tDown.addEventListener('click', function() {
      tvp.scrollBy({
        top: 80,
        behavior: 'smooth'
      });
    ...
    (function() {
      ...
      el.addEventListener('touchend', function(e) {
        ...
        if (Math.abs(dx) > 50) {
          dx < 0 ? setImage((activeIdx + 1) % IMAGES.length) : setImage((activeIdx - 1 + IMAGES.length) % IMAGES.length);
        }
        sx = null;
      }, {
        passive: true
    ...
    if (lbPrev) lbPrev.addEventListener('click', function() {
      openLightbox((activeIdx - 1 + IMAGES.length) % IMAGES.length);
    });
    if (lbNext) lbNext.addEventListener('click', function() {
      openLightbox((activeIdx + 1) % IMAGES.length);
    });
    ...
    function makeHorizSlider(trackId, prevId, nextId) {
      ...
      track.addEventListener('touchend', function(e) {
        ...
        var dx = e.changedTouches[0].clientX - sx;
        if (Math.abs(dx) > 50) {
          dx < 0 ? (idx = Math.min(maxIdx(), idx + 1)) : (idx = Math.max(0, idx - 1));
          slide();
        }
        sx = null;
      }, {
